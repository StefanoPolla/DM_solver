'''        b.xlabel = ['0+1', '0-1'] # Fix labels
        b.ylabel = ['0+i1', '0-i1']
        b.zlabel = ['0', '1']
	wrapper classes to simulate without noise
	- two qubits (00,01,10,11)
	- two dots (00,01,10,11,Sl,Sr) (S = singlet)

	TODO: make functions uniform, add docstrings
'''
# NOTE: import c_solver before any other library
...
class double_dot_hamiltonian():
	'''
	Class to simulate the trajectory of a double dot sysyem with 2 electrons, B field, tunneling and MW/RF pulses
	'''
	def __init__(self, B_1, B_2, chargingE1, chargingE2, tunnel_coupling):
		...
		# H_bar = 1
		self.H_charg1 = np.array(list(qt.basis(6,4)*qt.basis(6,4).dag()))[:,0]
		self.H_charg2 = np.array(list(qt.basis(6,5)*qt.basis(6,5).dag()))[:,0]


		self.H_B_field1 = np.array(list(-qt.basis(6,0)*qt.basis(6,0).dag() - qt.basis(6,1)*qt.basis(6,1).dag() +
							qt.basis(6,2)*qt.basis(6,2).dag() + qt.basis(6,3)*qt.basis(6,3).dag()))[:,0]/2
		self.H_B_field2 = np.array(list(-qt.basis(6,0)*qt.basis(6,0).dag() + qt.basis(6,1)*qt.basis(6,1).dag() -
							qt.basis(6,2)*qt.basis(6,2).dag() + qt.basis(6,3)*qt.basis(6,3).dag()))[:,0]/2

		self.H_exchange = np.array(
			list(-qt.basis(6,1)*qt.basis(6,1).dag() -qt.basis(6,2)*qt.basis(6,2).dag() +
				qt.basis(6,1)*qt.basis(6,2).dag() + qt.basis(6,2)*qt.basis(6,1).dag()))[:,0]/2
		self.H_tunnel = np.array(list(qt.basis(6,1)*qt.basis(6,4).dag() - qt.basis(6,2)*qt.basis(6,4).dag() +
									qt.basis(6,1)*qt.basis(6,5).dag() - qt.basis(6,2)*qt.basis(6,5).dag() +
									qt.basis(6,4)*qt.basis(6,1).dag() - qt.basis(6,4)*qt.basis(6,2).dag() +
									qt.basis(6,5)*qt.basis(6,1).dag() - qt.basis(6,5)*qt.basis(6,2).dag()))[:,0]
	...
	def mw_pulse(self, freq, phase, rabi_f, t_start, t_stop, sigma_Gauss= None, RWA = True):
		self.H_mw_qubit_1 = np.array(list(qt.basis(6,0)*qt.basis(6,2).dag() + qt.basis(6,1)*qt.basis(6,3).dag()))[:,0]
		self.H_mw_qubit_2 = np.array(list(qt.basis(6,0)*qt.basis(6,1).dag() + qt.basis(6,2)*qt.basis(6,3).dag()))[:,0]
	...
	def awg_pulse_tc(self, amp, t_start, t_stop, filtering = True, plot = 0):
		...
		my_filter = [['Butt', 1, 300e6], ['Butt', 2, 380e6]]
		if filtering == False:
			my_filter = [] # NOTE: this is ignored in next line
	...
	def awg_pulse_tc_custom(self, mat, bandwidth=0, plot=0): # NOTE: bandwidth unused
		# tunnel couplings pulse
	...
	# visuals:
	def plot_pop(self):
		dd = np.array(list(qt.basis(6,0)*qt.basis(6,0).dag()))[:,0]
		du = np.array(list(qt.basis(6,1)*qt.basis(6,1).dag()))[:,0]
		ud = np.array(list(qt.basis(6,2)*qt.basis(6,2).dag()))[:,0]
		uu = np.array(list(qt.basis(6,3)*qt.basis(6,3).dag()))[:,0]
		s1 = np.array(list(qt.basis(6,4)*qt.basis(6,4).dag()))[:,0]
		s2 = np.array(list(qt.basis(6,5)*qt.basis(6,5).dag()))[:,0]
	...
	def save_pop(self, location):
		dd = np.array(list(qt.basis(6,0)*qt.basis(6,0).dag()))
		du = np.array(list(qt.basis(6,1)*qt.basis(6,1).dag()))[:,0]
		ud = np.array(list(qt.basis(6,2)*qt.basis(6,2).dag()))[:,0]
		uu = np.array(list(qt.basis(6,3)*qt.basis(6,3).dag()))[:,0]
		s1 = np.array(list(qt.basis(6,4)*qt.basis(6,4).dag()))[:,0]
	...
	def plot_expect(self):
		XI = np.array(list(qt.basis(6,1)*qt.basis(6,3).dag() + qt.basis(6,0)*qt.basis(6,2).dag() + qt.basis(6,2)*qt.basis(6,0).dag() + qt.basis(6,3)*qt.basis(6,1).dag()))[:,0]
		IX = np.array(list(qt.basis(6,0)*qt.basis(6,1).dag() + qt.basis(6,1)*qt.basis(6,0).dag() + qt.basis(6,2)*qt.basis(6,3).dag() + qt.basis(6,3)*qt.basis(6,2).dag()))[:,0]
		XX = np.array(list(qt.basis(6,0)*qt.basis(6,3).dag() + qt.basis(6,1)*qt.basis(6,2).dag() + qt.basis(6,2)*qt.basis(6,1).dag() + qt.basis(6,3)*qt.basis(6,0).dag()))[:,0]
		ZZ = np.array(list(qt.basis(6,0)*qt.basis(6,0).dag() - qt.basis(6,1)*qt.basis(6,1).dag() - qt.basis(6,2)*qt.basis(6,2).dag() + qt.basis(6,3)*qt.basis(6,3).dag()))[:,0]
		ZI = np.array(list(qt.basis(6,0)*qt.basis(6,0).dag() + qt.basis(6,1)*qt.basis(6,1).dag() - qt.basis(6,2)*qt.basis(6,2).dag() - qt.basis(6,3)*qt.basis(6,3).dag()))[:,0]
		IZ = np.array(list(qt.basis(6,0)*qt.basis(6,0).dag() - qt.basis(6,1)*qt.basis(6,1).dag() + qt.basis(6,2)*qt.basis(6,2).dag() - qt.basis(6,3)*qt.basis(6,3).dag()))[:,0]
		YY_elem = np.array(list(qt.tensor(qt.sigmay(), qt.sigmay())))[:,0]
...
class two_qubit_sim():
    ...
    def mw_pulse(self, freq, phase, rabi_f, t_start, t_stop):
        ...
        self.H_mw_qubit_1 = np.array(list(qt.basis(4,0)*qt.basis(4,2).dag() + 
                                          qt.basis(4,1)*qt.basis(4,3).dag()))[:,0]
        self.H_mw_qubit_2 = np.array(list(qt.basis(4,0)*qt.basis(4,1).dag() + 
    ...
    def plot_expect(self):
        XI = np.array(list(qt.basis(4,1)*qt.basis(4,3).dag() + qt.basis(4,0)*qt.basis(4,2).dag() + 
                           qt.basis(4,2)*qt.basis(4,0).dag() + qt.basis(4,3)*qt.basis(4,1).dag()))[:,0]
        IX = np.array(list(qt.basis(4,0)*qt.basis(4,1).dag() + qt.basis(4,1)*qt.basis(4,0).dag() + 
                           qt.basis(4,2)*qt.basis(4,3).dag() + qt.basis(4,3)*qt.basis(4,2).dag()))[:,0]
        XX = np.array(list(qt.basis(4,0)*qt.basis(4,3).dag() + qt.basis(4,1)*qt.basis(4,2).dag() + 
                           qt.basis(4,2)*qt.basis(4,1).dag() + qt.basis(4,3)*qt.basis(4,0).dag()))[:,0]
        ZZ = np.array(list(qt.basis(4,0)*qt.basis(4,0).dag() - qt.basis(4,1)*qt.basis(4,1).dag() - 
                           qt.basis(4,2)*qt.basis(4,2).dag() + qt.basis(4,3)*qt.basis(4,3).dag()))[:,0]
        ZI = np.array(list(qt.basis(4,0)*qt.basis(4,0).dag() + qt.basis(4,1)*qt.basis(4,1).dag() - 
                           qt.basis(4,2)*qt.basis(4,2).dag() - qt.basis(4,3)*qt.basis(4,3).dag()))[:,0]
        IZ = np.array(list(qt.basis(4,0)*qt.basis(4,0).dag() - qt.basis(4,1)*qt.basis(4,1).dag() + 
                           qt.basis(4,2)*qt.basis(4,2).dag() - qt.basis(4,3)*qt.basis(4,3).dag()))[:,0]
    ...
    def plot_ST_bloch_sphere(self, npoints = 100): # NOTE: changed to allow resolution
        mat = self.solver_obj.get_all_density_matrices()[:,1:3,1:3]
    ...
    def plot_qubit_bloch_sphere(self, npoints = 100, qubit=0, fig=None):
        ...
        k = np.linspace(0, len(mat)-1, npoints, dtype=np.int)
        b=qt.Bloch(fig=fig)
        b.xlabel = ['0+1', '0-1'] # TODO: Verify/fix labels according to used convention
        b.ylabel = ['0+i1', '0-i1']
        b.zlabel = ['0', '1']
